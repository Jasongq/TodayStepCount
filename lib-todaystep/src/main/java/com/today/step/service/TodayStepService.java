...
import com.today.step.TodayStepCounter;
import com.today.step.TodayStepDetector;
import com.today.step.bean.TodayStepData;
import com.today.step.helper.TodayStepDBHelper;
import com.today.step.jlogger.JLoggerConstant;
import com.today.step.jlogger.JLoggerWraper;
import com.today.step.listener.ITodayStepDBHelper;
import com.today.step.listener.OnStepCounterListener;
import com.today.step.receiver.BaseClickBroadcast;
import com.today.step.util.DateUtil;
import com.today.step.util.NotificationApiCompat;
import com.today.step.util.SportStepJsonUtil;
import com.today.step.util.StepUtil;
import com.today.step.util.WakeLockUtil;
...
import com.today.step.ISportStepInterface;
/**
 * @author : GuoQiang
 * e-mail : 849199845@qq.com
 * time   : 2022/01/07  15:33
 * desc   : 当日步数服务
 * version: 1.0
 */
...
//        JLogger.i(TAG, "onStartCommand CurrStep :" + CURRENT_STEP + "   mSeparate:" + mSeparate + "    mBoot:" + mBoot);
    ...
    private synchronized void initNotification(int currentStep) {
        ...
        String km = SportStepJsonUtil.getDistanceByStep(currentStep);
        String calorie = SportStepJsonUtil.getCalorieByStep(currentStep);
        ...
        mNotificationApiCompat = new NotificationApiCompat.Builder(this,
                nm, STEP_CHANNEL_ID,
                getString(R.string.step_channel_name), smallIcon)
    ...
    private PendingIntent getPendingIntent() {
        return PendingIntent.getBroadcast(this, BROADCAST_REQUEST_CODE, new Intent(),
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
    }
...
//        JLogger.i(TAG, "cleanDb");
    ...
    private synchronized void updateNotification(int stepCount) {
        if (null != mNotificationApiCompat) {
            String km = SportStepJsonUtil.getDistanceByStep(stepCount);
            String calorie = SportStepJsonUtil.getCalorieByStep(stepCount);
            String contentText = calorie + " 千卡  " + km + " 公里";
    ...
    private final ISportStepInterface.Stub mIBinder = new ISportStepInterface.Stub() {
        ...
        private JSONArray getSportStepJsonArray(List<TodayStepData> todayStepDataArrayList) {
            return SportStepJsonUtil.getSportStepJsonArray(todayStepDataArrayList);
        }
