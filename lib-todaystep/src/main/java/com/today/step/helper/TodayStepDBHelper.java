...
public class TodayStepDBHelper extends SQLiteOpenHelper implements ITodayStepDBHelper {
    ...
    public synchronized List<TodayStepData> getStepListByStartDateAndDays(String startDate, int days) {
        ...
        for (int i = 0; i < days; i++) {
            ...
            calendar.setTimeInMillis(DateUtil.getDateMillis(startDate, DATE_PATTERN_YYYY_MM_DD));
            calendar.add(Calendar.DAY_OF_YEAR, i);
            Cursor cursor = getReadableDatabase().rawQuery(SQL_QUERY_STEP_BY_DATE,
                    new String[]{DateUtil.dateFormat(calendar.getTimeInMillis(), DATE_PATTERN_YYYY_MM_DD)});
    ...
    public synchronized void clearCapacity(String curDate, int limit) {
        ...
        try {
            Calendar calendar = Calendar.getInstance();
            calendar.setTimeInMillis(DateUtil.getDateMillis(curDate, DATE_PATTERN_YYYY_MM_DD));
            calendar.add(Calendar.DAY_OF_YEAR, -(mLimit));
            String date = DateUtil.dateFormat(calendar.getTimeInMillis(), DATE_PATTERN_YYYY_MM_DD);
            Log.e(TAG, date);
            ...
            for (TodayStepData tmpTodayStepData : todayStepDataList) {
                long dbTodayDate = DateUtil.getDateMillis(tmpTodayStepData.getToday(), DATE_PATTERN_YYYY_MM_DD);
